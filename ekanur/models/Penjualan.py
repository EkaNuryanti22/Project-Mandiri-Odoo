...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='ekanur.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for line in self:
            result = sum(self.env['ekanur.detailpenjualan'].search(
                [('penjualan_id', '=', line.id)]).mapped('subtotal'))
            line.total_bayar = result


class DetailPenjualan(models.Model):
    _name = 'ekanur.detailpenjualan'
    _description = 'Detail'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(
        comodel_name='ekanur.penjualan',
        string='Detail Penjualan')
    barang_id = fields.Many2one(
        comodel_name='ekanur.barang',
        string='List Barang')
    ...
    def __ondelete_penjualan(self):
        if self.detailpenjualan_ids:
            penjualan = []
            for line in self:
                penjualan = self.env['ekanur.detailpenjualan'].search(
    ...
    def write(self, vals):
        for line in self:
            data_asli = self.env['ekanur.detailpenjualan'].search([('penjualan_id', '=', line.id)])
            print(data_asli)
        ...
        for line in self:
            data_setelah_edit = self.env['ekanur.detailpenjualan'].search([('penjualan_id', '=', line.id)])
            print(data_asli)
